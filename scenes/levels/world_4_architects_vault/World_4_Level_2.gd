...
func setup_level() -> void:
	...
	var key_block = $KeyBlock
	var ghost_outline = $GhostOutline

	# Create alignment condition: block must match ghost outline position, rotation, AND scale
	var alignment_condition = AlignmentCondition.new()
	alignment_condition.target_object = key_block
	alignment_condition.target_transform = Transform2D(deg_to_rad(90.0), ghost_outline.global_position)
	alignment_condition.target_scale = Vector2(1.5, 1.5)
	alignment_condition.position_tolerance = 10.0
	alignment_condition.rotation_tolerance = 5.0
	alignment_condition.scale_tolerance = 0.1

	completion_conditions = [alignment_condition]
