...
func setup_level() -> void:
	...
	var target_alignment = $TargetAlignment

	# Create alignment condition: block must match ghost outline position and rotation
	var alignment_condition = AlignmentCondition.new()
	alignment_condition.target_object = key_block
	alignment_condition.target_transform = Transform2D(deg_to_rad(45.0), ghost_outline.global_position)
