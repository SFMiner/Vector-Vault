...
func _ready() -> void:
	...
	sequence_recorder = $UI/SequenceRecorder
...
func _connect_button_hover() -> void:
	...
	radial_menu.get_node("RotateButton").mouse_entered.connect(func():
		preview.show_preview(target, "rotate", {"angle": 45.0})
	)
...
func _on_transformation_selected(type: String, params: Dictionary = {}) -> void:
	...
	if not params.is_empty():
		...
		# Record step in sequence recorder
		sequence_recorder.record_step(type, final_params)
...
func _on_parameter_confirmed(transformation_type: String, params: Dictionary) -> void:
	...
	# Record step in sequence recorder
	sequence_recorder.record_step(transformation_type, final_params)
...
func setup_level() -> void:
	...
	sequence_recorder.enable_recording()

	var block1 = $Block1
	var block2 = $Block2
	var block3 = $Block3
	var ghost_outline1 = $GhostOutline1
	var ghost_outline2 = $GhostOutline2
	var ghost_outline3 = $GhostOutline3
	var pressure_plate = $PressurePlate
	var door = $Door

	# Create alignment conditions for each block
	var alignment_condition1 = AlignmentCondition.new()
	alignment_condition1.target_object = block1
	alignment_condition1.target_transform = Transform2D(0.0, ghost_outline1.global_position)
	alignment_condition1.position_tolerance = 15.0
	alignment_condition1.rotation_tolerance = 5.0
	alignment_condition1.scale_tolerance = 0.15

	var alignment_condition2 = AlignmentCondition.new()
	alignment_condition2.target_object = block2
	alignment_condition2.target_transform = Transform2D(0.0, ghost_outline2.global_position)
	alignment_condition2.position_tolerance = 15.0
	alignment_condition2.rotation_tolerance = 5.0
	alignment_condition2.scale_tolerance = 0.15

	var alignment_condition3 = AlignmentCondition.new()
	alignment_condition3.target_object = block3
	alignment_condition3.target_transform = Transform2D(0.0, ghost_outline3.global_position)
	alignment_condition3.position_tolerance = 15.0
	alignment_condition3.rotation_tolerance = 5.0
	alignment_condition3.scale_tolerance = 0.15

	completion_conditions = [alignment_condition1, alignment_condition2, alignment_condition3]

	# Connect pressure plate to door
	pressure_plate.activated.connect(func():
		door.open()
	)
	pressure_plate.deactivated.connect(func():
		door.close()
	)
